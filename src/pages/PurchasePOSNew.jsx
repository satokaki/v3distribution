...
export default function PurchasePOSNew() {
  ...
  const [master, setMaster] = useState({ suppliers: [], products: [], warehouses: [], accounts: [] });
  ...
  useEffect(() => {
    ...
    (async () => {
      setLoading(true);
      try {
        const [suppliers, products, warehouses, accounts] = await Promise.all([
        ...
        if (!cancelled) {
          const next = { suppliers: (suppliers || []).filter((x) => x.is_active !== false), products: (products || []).filter((x) => x.is_active !== false), warehouses: warehouses || [], accounts: accounts || [] };
          setMaster(next); setWarehouseId(next.warehouses[0]?.id || ""); setAccountId(next.accounts.find((x) => x.account_type === "kas")?.id || next.accounts[0]?.id || ""); await loadDrafts();
        }
  ...
  const warehouse = master.warehouses.find((x) => x.id === warehouseId) || master.warehouses[0];
  ...
  const openDraft = (draft) => { setEditingDraftId(draft.id); setSupplierId(draft.supplier_id || ""); setWarehouseId(draft.warehouse_id || ""); setAccountId(draft.account_id || ""); setDate(draft.date || localDate()); setDueDate(draft.due_date || ""); setMethod(draft.payment_method || "kredit"); setChannel(draft.payment_channel || "cash"); setNote(draft.note || ""); setItems((draft.items || []).map((x) => ({ ...x, discount_percent: x.discount_percent || 0 }))); };
  ...
  return <div className="space-y-4 pb-28 lg:pb-0">
    ...
    <div className="grid gap-4 xl:grid-cols-[minmax(0,1fr)_340px]"><main className="overflow-hidden rounded-2xl border bg-card shadow-sm"><div className="grid gap-4 border-b p-5 md:grid-cols-3">
      ...
      {master.warehouses.length > 1 ? <label className="space-y-1.5"><span className="text-sm font-medium">Gudang</span><select value={warehouseId} onChange={(e) => setWarehouseId(e.target.value)} className={fieldClass}>{master.warehouses.map((x) => <option key={x.id} value={x.id}>{x.name}</option>)}</select></label> : <div className="space-y-1.5"><span className="text-sm font-medium">Gudang</span><div className={`${fieldClass} flex items-center`}>{warehouse?.name || "Belum tersedia"}</div></div>}
