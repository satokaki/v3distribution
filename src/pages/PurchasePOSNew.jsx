...
import { Barcode, Building2, FileText, Minus, Plus, ReceiptText, RefreshCcw, Save, Search, ShoppingCart, Trash2 } from "lucide-react";
...
export default function PurchasePOSNew() {
  ...
  const [master, setMaster] = useState({ suppliers: [], products: [], accounts: [], branch: null });
  const activeBranch = master.branch || operationalBranch;
  const isHeadOfficeBranch = master.branch?.branch_type === "pusat";
  ...
  useEffect(() => {
    ...
    (async () => {
      setLoading(true);
      try {
        setMaster({ suppliers: [], products: [], accounts: [], branch: null });
        setDrafts([]); setItems([]); setEditingDraftId("");
        const branch = await base44.entities.Branch.get(branchId);
        if (!branch || branch.is_active === false || branch.branch_type !== "pusat") {
          if (!cancelled) setMaster({ suppliers: [], products: [], accounts: [], branch: branch || null });
          return;
        }
        ...
        if (!cancelled) {
          const next = { suppliers: (suppliers || []).filter((x) => x.is_active !== false), products: (products || []).filter((x) => x.is_active !== false), accounts: accounts || [], branch };
  ...
  const deleteDraft = async (draft) => { await deletePurchaseDraft(draft.id); await loadDrafts(); toast({ title: `Draft ${draft.code} dihapus` }); };

  if (loading) return <div className="py-20 text-center text-sm text-muted-foreground">Menyiapkan terminal pembelian...</div>;
  if (!isHeadOfficeBranch) return <div className="flex min-h-[60vh] items-center justify-center p-6"><div className="max-w-xl rounded-2xl border border-amber-200 bg-amber-50 p-8 text-center shadow-sm"><div className="mx-auto mb-4 flex h-12 w-12 items-center justify-center rounded-xl bg-amber-500/15 text-amber-700"><Building2 className="h-6 w-6" /></div><h1 className="text-xl font-semibold text-amber-950">Pembelian supplier hanya dapat dilakukan dari Pusat.</h1><p className="mt-2 text-sm text-amber-800">Cabang retail memperoleh stok melalui Mutasi dari Pusat. Pilih Operational Branch bertipe Pusat melalui Branch Selector untuk membuat pembelian supplier.</p><div className="mt-5 inline-flex rounded-lg border border-amber-300 bg-white px-4 py-2 text-sm font-semibold text-amber-900">Pilih Cabang Pusat di Branch Selector</div></div></div>;
